---
name: IQS323 터치센서 레지스터 레퍼런스
purpose: Azoteq IQS323 레지스터 설정 참조
type: 참고
applies_to: [Sound1]
tags: [chip, iqs323, touch, register]
---

# IQS323 터치센서 레지스터 레퍼런스

**TL;DR**: Azoteq IQS323 ProxFusion 터치 IC의 I2C 통신·이벤트 모드·ATI·채널 설정 레지스터를 데이터시트 v1.2(2022-11)에서 발췌해 비트필드 단위로 정리한 펌웨어 참조 문서.
