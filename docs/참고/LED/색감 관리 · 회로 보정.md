---
name: LED 시스템 기술 문서 — 색감 관리 · 회로 보정
purpose: RGB LED 색감 교정(채널별 cap·보드 튜닝)
type: 참고
applies_to: [Sound1]
tags: [led, color, calibration, hardware]
---

# LED 시스템 기술 문서 — 색감 관리 · 회로 보정

**TL;DR**: LRTBR48G + 1.2 kΩ + N-FET low-side 회로의 채널별 밝기 불균형을 색상별 R/G/B duty cap 테이블로 교정하는 아키텍처를 다루며, Board_OTE_ver1_5 실측 튜닝 절차·HW 한계·개선 방향을 정리.
...
Dimming 엔진 (fade 타이밍, LUT) 은 [`Dimming · 인지 곡선 · LUT`](Dimming%20·%20인지%20곡선%20·%20LUT.md) 참조. 전체 아키텍처는 [`아키텍처 · 운용`](아키텍처%20·%20운용.md) 참조.
...
Ezairo 8300 TIMER 는 **HW PWM 출력 기능 없음** ([`Ezairo 8300 클럭·타이머·I2C 스펙 정리`](../칩/Ezairo%208300%20클럭·타이머·I2C%20스펙%20정리.md) §2.1 참조). 모드가 Single-shot / Multi-shot / Free-run / DIO 인터럽트 캡처 뿐 — **Output Compare / PWM 출력 핀 없음**.
...
- [`아키텍처 · 운용.md`](아키텍처%20·%20운용.md) — 전체 아키텍처
- [`Dimming · 인지 곡선 · LUT.md`](Dimming%20·%20인지%20곡선%20·%20LUT.md) — fade · LUT 내부
- [`Ezairo 8300 클럭·타이머·I2C 스펙 정리.md`](../칩/Ezairo%208300%20클럭·타이머·I2C%20스펙%20정리.md) — TIMER 스펙 (후속 PWM 해상도 향상 시 참고)
