---
name: LED 시스템 기술 문서 — 아키텍처 · 운용
purpose: LED 인디케이터 시스템 계층·arbiter·상태/소스
type: 참고
applies_to: [Sound1]
tags: [led, architecture, arbiter, state]
---

# LED 시스템 기술 문서 — 아키텍처 · 운용

**TL;DR**: E8300 RGB LED 인디케이터 시스템의 레이어 구조(소스→arbiter→dimming→PWM), 6개 소스(power/error/BLE/mapping/battery/ISD)와 우선순위, 100 Hz 소프트 PWM ISR, 부팅·절전 시퀀스를 한데 모은 시스템 종합 문서.
...
Dimming 엔진 내부 (CIE L* LUT, fade 곡선) 는 별도 문서 [`Dimming · 인지 곡선 · LUT`](Dimming%20·%20인지%20곡선%20·%20LUT.md) 참조. 색감 교정 (per-color cap 테이블) 은 [`색감 관리 · 회로 보정`](색감%20관리%20·%20회로%20보정.md) 참조.
...
상세 회로·LED 특성 · 색감 보정은 [색감 관리 문서](색감%20관리%20·%20회로%20보정.md) 참조.
...
단색 3종 + 조합색 4종 + BLACK. 각 색상의 R/G/B PWM duty cap 은 `k_led_mix[]` 에서 결정 ([색감 문서](색감%20관리%20·%20회로%20보정.md) 참조).
...
brightness (perceived) 는 `calc_perceived_pattern(timer_ms, on_ms, period_ms)` 로 산출 — fade-in/peak/fade-out 프로파일 적용 ([Dimming 문서](Dimming%20·%20인지%20곡선%20·%20LUT.md) 참조).
...
상세 사용법: [`UI 명령어 사용법.md`](../../사용방법/UI%20명령어%20사용법.md)
...
- [`Dimming · 인지 곡선 · LUT.md`](Dimming%20·%20인지%20곡선%20·%20LUT.md) — fade 곡선 / CIE L* LUT / cross-fade 내부
- [`색감 관리 · 회로 보정.md`](색감%20관리%20·%20회로%20보정.md) — 회로 · 색감 cap 테이블 · 튜닝 절차
- [`UI 명령어 사용법.md`](../../사용방법/UI%20명령어%20사용법.md)
- [`Ezairo 8300 클럭·타이머·I2C 스펙 정리.md`](../칩/Ezairo%208300%20클럭·타이머·I2C%20스펙%20정리.md)
