...
├── 터치 임계·계수 레지스터 동작/ 코드 상수↔레지스터 계산·적용 레퍼런스 (md 상세 + html 요약)
├── 개선/                       제어 재설계·검토 산출물
...
## 임계·계수 레지스터 동작 (코드↔레지스터 레퍼런스)

코드 상수가 IQS323 레지스터에서 어떻게 계산·적용되는지 전수 정리. 페르소나 4 + adversarial 검증 1로 교차검증(할루시네이션 0).

| 문서 | 내용 |
|---|---|
| [터치 임계·계수 레지스터 동작.md](터치%20임계·계수%20레지스터%20동작/터치%20임계·계수%20레지스터%20동작.md) (상세) · [index.html](터치%20임계·계수%20레지스터%20동작/index.html) (한눈 요약) | THRESHOLD/HYSTERESIS/PROX/Beta/ATI 상수→레지스터 매핑(그대로/코드변환/IC계산)·IC 내부 계산식(절대임계=계수×LTA/256 등)·계수 출처·데이터흐름·Beta 해석(AZD004 실측확정)·grounding·실측게이트 4종 |
