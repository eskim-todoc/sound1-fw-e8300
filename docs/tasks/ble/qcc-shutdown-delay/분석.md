...
- [`signalProcessing/nofm-merge`](../../signalProcessing/nofm-merge/) — 본 task 분리 출처 (NofM 분석 §6.1).
- [`bootloader/service-rtt-debug-console`](../../bootloader/service-rtt-debug-console/) — 부트로더 측 OTA DFU 흐름 (`bootloader_boot()` storage init + slot 적재).
- [`LED/ota-dfu-fw-variant`](../../LED/ota-dfu-fw-variant/) — OTA fw variant 분기 (APP / FACTORY_RESET 슬롯).
- [`Sullivan1.5 rel.2 sync`](../../sync/sullivan1.5-rel2/) 의 [`분석 BleCommunication.md`](../../sync/sullivan1.5-rel2/분석%20BleCommunication.md) §4 (Sound1 후속 작업 인벤토리) — QCC SHUTDOWN 시퀀스 변경이 Sound1 측 추가분으로 명시되어 있음 (Sullivan 베이스 ↔ Sound1 비교 시 발견).
