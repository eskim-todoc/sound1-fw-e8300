...
| 부트로더 단계 LED 조기 점등 | LED | bootloader, startup, indicator | [LED/bootloader-power-on-indicator](LED/bootloader-power-on-indicator/) | 진행 | 2026-04-27 | ← [LED/non-blocking-fade-off](LED/non-blocking-fade-off/) (A·B 단계), ↔ [LED/power-on-early-lighting](LED/power-on-early-lighting/) (별개 진행) | Step 1 (`4099af7`, 가드+빈 stub) + Step 2 (`75fa380`, Timer2 ISR + phase 머신 본체) commit 완료. 사용자 Eclipse 빌드·하드웨어 측정 대기 (회귀: 가드 OFF / 동작: 가드 ON × UART 비활성·활성). 진행상황.md §6 안내 |
