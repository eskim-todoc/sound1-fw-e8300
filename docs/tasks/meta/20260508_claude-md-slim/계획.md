...
| 지침 | 준수 |
|---|---|
| 루트 `문서 작성 규칙.md` §9 frontmatter+TL;DR | 완료 본 계획·신규 CLAUDE.md 모두 |
| 루트 §6.5 의미 한글 라벨 | 완료 `검증_1`~`검증_7` 매핑 |
| 루트 §6.3 Mermaid | 완료 §1 적용 절차 |
| 분석.md 결정 사항 반영 | 완료 결정_1·2·3·4 모두 본문에 |

### 4.2 논리적 정합성

| 점검 | 결과 |
|---|---|
| 신규 본문이 요구사항 모두 만족하는가 | 완료 검증 매핑 완전 |
| 줄 수 감소 vs 정보 손실 | 완료 운영 정보(외부 의존)만 제거, 작업 컨텍스트는 보존 |
| 머지 절차 누락 없는가 | 완료 적용 절차 ①~⑥ 완비 (stash pop·원 브랜치 복귀까지) |
| 루트 우선/프로젝트 우선 분리 모순 없는가 | 완료 CLAUDE.md ↔ CLAUDE.md = 루트 우선, `docs/지침/` ↔ `docs/지침/` = 프로젝트 우선 |
