...
| 8 | option 4 분기 | `tdc_remote_general_debug.c:51~53` | 3줄 |
...
| 순서 | 단계 | 전제 | 검증 |
|---|---|---|---|
| 1 | BLE 진입점 제거 (#7, #8) | 없음 | option 4 도달 불가 확인 |
| 2 | `func_normal()` 호출 분기 제거 (#2) | 단계_1 | `fake_func_sleep()` 호출 0 |
| 3 | `fake_func_sleep()` 정의 제거 (#1) | 단계_2 | 심볼 잔존 0 |
| 4 | op_mode getter/setter 제거 (#3, #4) | 단계_3 | 심볼 잔존 0 |
| 5 | `ci_fake_power_sleep()` 제거 (#5, #6) | 단계_3 | 심볼 잔존 0 |

**커밋 1개**로 묶는다 — 단계 간 중간 상태가 "선언은 있는데 정의가 없는" 불완전 상태라 이분 탐색 이점이 없다.
...
| 검증_2 | `func_sleep()` 무영향 | 헬퍼 5개 + `ci_power_sleep()` 호출 그대로 |
| 검증_3 | 브레이스 균형 | `main.c` / `ci_power.c` / `tdc_remote_general_debug.c` |
| 검증_4 | 전처리기 균형 | `main.c` (fake 내부에 `#if 1` / `#if 0` 존재) |
| 검증_5 | BLE 응답 | option 1/2/3 정상 동작 (회귀 없음) |
| 검증_6 | 인코딩 | pre-commit 훅 통과 |
| 검증_7 | 빌드 | 은수님 Eclipse |
