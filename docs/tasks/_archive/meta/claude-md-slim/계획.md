...
**TL;DR**: 현행 99줄·9섹션 → 신규 약 50~60줄·6섹션. frontmatter+TL;DR 추가, 헤더 Alert 강화, 루트 지침 참조 표·외부 의존 3섹션·문서 작성 다이어그램 제거, list.md 진입점 별도 §작업 라우팅으로, 절대 경로 → 상대 통일.
...
```mermaid
flowchart LR
    A["① CLAUDE.md Write<br/>(신규 본문)"] --> B["② 진행상황·이력.md 갱신"]
    B --> C["③ git add + commit"]
    C --> D["④ claude_develop 체크아웃<br/>--no-ff 머지"]
    D --> E["⑤ feature 삭제 + archive 이동<br/>(이력.md 병합 해시 갱신)"]
