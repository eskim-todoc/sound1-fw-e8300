---
name: CFX-CM3 공유메모리 시퀀스 및 타이밍 Rev.1
purpose: 두 코어 PRAM5 위 데이터 교환 시퀀스·타이밍 ms 단위 분해
type: tasks/분석
applies_to: [Sound1]
tags: [signalProcessing, cfx, cm3, shared-memory, timing, sequence, mermaid, legacy-2026-04-18, rev1]
---

# CFX–CM3 공유메모리 시퀀스 및 타이밍 Rev.1

**TL;DR**: Rev.0 다이어그램을 Mermaid로 재작성 + CM3 writer/reader 양방향 매트릭스 + 부팅 순서(부트로더 → CFX → CM3, 수십 μs 간격) + 1 msec PCM 틱 동기 모델 추가. 핵심 3 사실 — ① 두 코어 main-loop 틱이 FIFO FA0_4 1 msec 인터럽트로 동기 (새 시그널 최소 지연 1 ms), ② 현 맵 변경은 3-way 핸드셰이크 3~4 ms (왕복 2회 → 1회 축소 시 2 ms 단축 가능), ③ SpecificCommand race 보호 플래그가 CFX에만 있고 CM3 polling 누락. 시퀀스 축소 제안 9건 + 타이밍 버짓 표.
