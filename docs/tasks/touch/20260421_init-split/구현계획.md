...
| 항목 | 상태 | 비고 |
|---|---|---|
| 분할 구조 채택 | 완료 확정 | `begin()` / `finish()` 2단계 |
| `begin()` / `finish()` 주체 | 완료 확정 | 둘 다 메인 루프 |
| `begin()` 호출 위치 | 완료 확정 | 옵션 A — `systemControl.c:210` 분기 |
| `finish()` 스텝 분할 | 완료 확정 | 일괄 진행 (~50ms 블록, tick 내에서 처리 가능) |
| 파워온 직후 롱터치 UX | 완료 불필요로 확정 | 9절 전제 3 |
| 구현 착수 | 완료 승인 | 2026-04-20 |
