...
> [!NOTE]
> "선행 블록"에 들어간 단계 중 일부 (18 DMA / 7 map_init) 는 사실 **LED 점등이 **요구** 하지 않는다 (DMA/map 은 LED GPIO 와 무관).** 그러나 C2 제약 (map → ISD 복사) · H3 제약 (인터럽트) 등 때문에 LED 이전에 끝내야 한다. 병렬 가능 블록은 14 가지가 남는다.
...
|---|---|---|---|---|
| A (TIMER3 재활용) | **낮음** | 낮음 (전환만 주의) | 수십~수백 ms | **우선 후보** |
| B (TIMER2 신규) | 중 | 낮음 (분리 명확) | A 와 동등 | A 로 얻기 어려운 가치 없음 |
| C (타이머 없이) | 낮음 | 높음 (CFX 경쟁) | 제한적 | 배제 |

> [!NOTE]
> **권고안: 옵션 A** (§6 참조). 다만 TIMER3 재활용 시 ULP 타이머와의 라이프사이클 중복 주의 — Initialize 진입 시 init, CFX iteration 활성화 직전에 uninit. func_sleep() 진입 시점에는 이미 uninit 상태여야 함.
...
| P3 | Initialize 선행 블록 완료 직후 (§2.2 PreLED 의 마지막 A10 다음) | H1~H4 모두 만족. **가장 안전한 최조기 시점** |
...
| # | 위험 | 시나리오 | 대응 |
|---|---|---|---|
| R1 | TIMER3 + CFX FIFO 동시 활성 → 틱 2 배 | Initialize 중 `ci_timer_init` 후 uninit 타이밍 실수 | **엄격한 순서 로깅 + RTT 로 확인**. `ci_timer_uninit()` 호출 후 `is_enabled_CFX_iteration = 1` 까지의 구간에 배리어 로그 삽입 |
...
| R6 | ULP 모드 복귀 후 (함수 `func_sleep` 진입 → 깨어나서 `func_normal` 재진입) TIMER3 상태가 남아있어 이중 init | `func_normal` 재진입에서도 `ci_timer_init` 가 안전한지 확인 필요 (`Sys_Timer_Stop` 선행) | `ci_timer_init_prescaled()` 가 이미 `Sys_Timer_Stop` 선행 ([ci_timer.c:151](../src/2__cm3/Gen1_5/common/ci_timer.c)) → 안전 |
...
| # | 질문 | 분석 근거 섹션 | 권고 후보 |
|---|---|---|---|
| Q1 | 1 ms 틱 소스 | §5 | **옵션 A (TIMER3 재활용)** — 기존 ISR 가 이미 준비됨, 전환 규칙 단순 |
| Q2 | LED 요청 위치 | §6 | **P3** — Initialize 선행 블록 이후, Initialize 내부에 요청 배치 |
| Q3 | CFX 시작 대기 스핀 | §7 | 스핀 유지. 단 그 앞에 `ci_power_normal / ci_dio_configure_normal / ci_timer_init` 이동하여 스핀 중 LED 가 이미 돌게 함 (성능 대비 효과는 작으므로 옵션) |
| Q4 | Initialize 분할 | §2 · §6 | 물리적 분할 아니오 — `Initialize()` 내부에서 **선행 블록 → LED 요청 → 병렬 블록 → 최종 CFX iteration 활성화** 순으로 재배열만 수행. 호출자(`main.c`) 구조 변경 없음 |
| Q5 | `tdc_touch_init_begin()` 통합 | §8 | LED 요청 바로 뒤에 유지 (현 systemControl.c 의 패턴을 initialize.c 로 이관). `try_finish_init()` 은 기존처럼 메인 루프의 `tdc_touch_process()` 에서 처리 |
