---
name: 이력 — LED 운용 방식 Rev.5 처리 결과
purpose: 부팅 직후 노란색 LED 점등(BATT_CRITICAL 오인식) 원인 분석·수정 결과 기록
type: tasks/이력
applies_to: [Sound1]
maturity: stable
tags: [LED, arbiter, batt-critical, boot-sequence, history]
---

# LED 운용 방식 Rev.5 처리 결과

**TL;DR**: 부팅 직후 "노란색 LED 잠깐 점등" 미정의 동작 원인은 `Initialize()`가 배터리를 pct=0/RESET으로 초기화하고 메인 루프 1st iter가 `pct < 10`만 보고 `LED_ST_BATT_CRITICAL`(prio 60)을 요청해 ORANGE 점멸이 표시되는 것. 후속 fake 배터리 주입(pct=90)·POWER_ON 요청으로 SKYBLUE 버스트가 덮어쓰는 시퀀스 timeline을 분석하고 Rev.5에서 수정.

작성자: 김은수
작성일: 2026-04-15
상태: 구현 1차 완료. 실기 검증 대기.
브랜치: `claude_led-rev5-impl`
관련 분석서: [`tasks/LED/20260421_operation-scheme/분석.md`](분석.md)
