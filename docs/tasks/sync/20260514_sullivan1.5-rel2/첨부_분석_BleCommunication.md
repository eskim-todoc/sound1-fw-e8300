...
**해석**: Sound1 의 service RTT 콘솔 작업([`bootloader/20260507_service-rtt-debug-console`](../../bootloader/20260507_service-rtt-debug-console/)) 또는 그 이전 로그 정리 단계에서 본 DFU 진단 로그까지 함께 제거된 것으로 추정. OTA DFU 실패 시 원인 추적용으로 유용한 로그이므로, 적절한 채널(예: `RTT_printf` 또는 같은 작업에서 도입한 BLOCK 모드 RTT)로 부활시킬지 검토 필요.
