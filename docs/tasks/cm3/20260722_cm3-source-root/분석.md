...
| 현행 | 신규 | 파일 |
|---|---|---|
| `hal` `drv` `led` `touch` `ui` `sys` `pwr` `fs` `ble` `qcc` `dfu` `isd` `stim` `cfx_link` `boot` `util` | `source/<동일>` | 140 |
...
계 **180항목**. 이동 후 프로젝트 루트에 남는 것: `source/` · `Debug/` · `sections.ld` · `.cproject` · `.project` · `.settings/` · `.gitignore`.
