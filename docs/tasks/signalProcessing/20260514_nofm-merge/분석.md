...
**별도 task 후보**: `ble/20260514_qcc-shutdown-delay` 또는 `ble/ota-qcc-shutdown` placeholder 등재. ble 모듈.
...
LED 모듈의 `LED/20260508_ota-dfu-fw-variant` 완료 task (`356a1e5` commit) 에서 도입한 빌드 분기. 본 변경은 default 를 `APP` → `FACTORY_RESET` 으로 전환 (LED_ST_IN_USE = PURPLE, LED_ST_POWER_ON = WHITE).
...
1. **CM3 측 별도 task 등재** (사용자 명시 자유 — 지금/나중):
   - `ble/20260514_qcc-shutdown-delay` (또는 `ble/ota-qcc-shutdown`) — `ci_ble_control_boot.c` OTA 후 QCC 절전 진입 딜레이 2200×1 ms.
