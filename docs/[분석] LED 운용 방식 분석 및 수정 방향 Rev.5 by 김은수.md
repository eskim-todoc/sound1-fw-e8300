# 2__cm3 LED 운용 분석 및 수정 방향 (Rev.5, 패턴/네이밍 정정본)

작성자: 김은수
작성일: 2026-04-15
최근 갱신: 2026-04-15 (표기 명확화. §1.5 MAPPING_NO_ISD 수치를 §변경점·§3.4와 일치하도록 정정)
상태: **구현 착수 확정본**. Rev.4의 LED 패턴/네이밍을 스펙에 맞게 정정.

> **표기 안내 (전체 문서 공통)**
> - 점멸 패턴은 모두 `ON X ms / OFF Y ms` 형식으로 표기. 합산 주기는 `X + Y` ms.
>   예) `ON 1100ms / OFF 1100ms` → 켜진 시간 1100ms, 꺼진 시간 1100ms, 한 주기 2200ms.
> - 점멸이 없으면 `지속 ON`.
> - 코드 디스크립터 `(on_ms, period_ms)` 와의 관계: `period_ms = on_ms + off_ms`.
>   예) `(on_ms=1100, period_ms=2200)` ↔ `ON 1100ms / OFF 1100ms`.

Rev.5 변경점:
- **`LED_ST_BATT_CRITICAL` (배터리 즉시 충전 필요)**: ON 180ms / OFF 180ms 점멸 → **ON 1100ms / OFF 1100ms 점멸**.
- **`LED_ST_BATT_MID` (배터리 완충 아님)**: ON 1100ms / OFF 1100ms 점멸 → **지속 ON (점멸 없음)**.
- **`LED_ST_READY` → `LED_ST_BATT_READY`로 이름 변경**: 배터리 상태 네이밍 일관성 확보(`BATT_READY` / `BATT_MID` / `BATT_CRITICAL`).
- **`LED_ST_OTA_EZAIRO`**: ON 1100ms / OFF 1100ms → **ON 180ms / OFF 180ms 점멸** (QCC와 시각적으로 구분).
- **`LED_ST_OTA_QCC`**: **ON 1100ms / OFF 1100ms 점멸** (변경 없음, 명시 재확인).
- **`LED_ST_MAPPING_NO_ISD`**: ON 300ms / OFF 300ms 점멸 → **ON 1100ms / OFF 1100ms 점멸**.
...
- 배터리 레벨 판정 (충/방전 무관):
  - `level ≥ 80%` → BATT_READY (녹색 지속 ON)
  - `10% ≤ level < 80%` → BATT_MID (노랑 지속 ON)
  - `level < 10%` → BATT_CRITICAL (노랑 ON 1100ms / OFF 1100ms 점멸)
- 히스테리시스 ±2% (BATT_CRITICAL 진입 <10%, 해제 ≥12%; BATT_READY 진입 ≥80%, 해제 <78%).
...
- `BLE_communicationState.mappingConnection == true` 일 때:
  - ISD 미연결 → 파랑 ON 1100ms / OFF 1100ms 점멸.
  - ISD 연결 → 파랑 지속 ON.
- BATT_CRITICAL / IN_USE / BATT_READY / BATT_MID를 선점.
...
| 4 | PAIR latch | 스펙 ** 주석 "인증 시간이 매우 짧아 LED 표시 불가능할 수 있음". 1주기(ON 180ms + OFF 180ms = 360ms) 확보 필요. | **최소 latch 500ms** (요청 해제 후에도 500ms 유지). |
...
| prio | 상태 | 패턴 | 비고 |
|---|---|---|---|
| 100 | POWER_OFF | BLUE ON 100ms / OFF 200ms × 4회 버스트 | 버스트 완료까지 유지 (게이트). Error 선점 |
| 95  | POWER_ON  | SKYBLUE ON 80ms / OFF 220ms × 5회 버스트 | 버스트 완료까지 유지 (게이트). 에러 조건에서는 요청 자체가 안 남 |
| 90  | ERROR_*   | 빨강 ON 180ms / OFF 180ms 점멸 | Power Off 외 전부 선점 |
| 80  | OTA_QCC   | 녹색 ON 1100ms / OFF 1100ms 점멸 | |
| 80  | OTA_EZAIRO | 녹색 ON 180ms / OFF 180ms 점멸 | QCC와 시각 구분 |
| 75  | MAPPING_ISD | 파랑 지속 ON | |
| 75  | MAPPING_NO_ISD | 파랑 ON 1100ms / OFF 1100ms 점멸 | |
| 70  | PAIR | 파랑 ON 180ms / OFF 180ms 점멸 | latch 500ms |
| 60  | BATT_CRITICAL | 노랑 ON 1100ms / OFF 1100ms 점멸 | 사용자 off 시 억제 |
| 40  | IN_USE | 흰색 지속 ON | |
| 30  | BATT_READY | 녹색 지속 ON | |
| 20  | BATT_MID | 노랑 지속 ON | |
| 10  | IDLE | OFF | |
| 0   | NONE | — | |

### 3.4 패턴 디스크립터
```c
/* on_ms     : 한 주기 내 LED ON 시간
 * period_ms : 한 주기 전체 = on_ms + off_ms.
 *             on_ms == 0 && period_ms == 0 → 지속 ON.
 * 사람이 읽는 'ON X ms / OFF Y ms' 와의 환산: period_ms = on_ms + off_ms. */
typedef struct {
    EN__LED_COLOR color;
    uint16_t      on_ms;
    uint16_t      period_ms;
...
- `batteryNPowerControl` 또는 `systemControl`에서 %값 기반 판정:
  ...
  else if (pct >= 80)                                 st = LED_ST_BATT_READY;
  else if (pct >= 78 && prev == LED_ST_BATT_READY)    st = LED_ST_BATT_READY;    // hyst
...
| `en__LED_StandbyForconneded_ISD_batteryNormal/Low` | 제거 → `LED_ST_BATT_READY` / `LED_ST_BATT_MID` / `LED_ST_BATT_CRITICAL` |
...
| `en__LED_Map_Error / MCU_Error / Accelerometer_Error / FPGA_Error / RF_PMIC_Error` | 타이밍 ON 180ms / OFF 180ms 통일. 소스 식별은 이벤트 로그 |
...
8. **사용자 LED off 검증**: BATT_CRITICAL / MAPPING / BATT_READY 등 억제, ERROR / POWER_* 표시 유지 확인.
9. **Dead enum 삭제** (`en__LED_ISD_*`, `MappingConneted_*`, `StandbyForconneded_*`, `BatteryChargingLevel_*`, `IND_BATT_STATE`).
10. **회귀 테스트**: Power On → BATT_READY 전이, Power Off 버스트 후 systemOff, 에러 주입, OTA, mapping 연결/ISD 연결 조합.
...
### 7.1 단위 (`tests/led/`)
- 디스크립터별 ON 시간 / 한 주기 오차 ±1ms (tick 시뮬).
...
### 7.2 통합 / HW
- 오실로스코프 파형 검증 (모두 ±5% 이내):
  - ON 180ms / OFF 180ms (PAIR, OTA_EZAIRO, ERROR_*)
  - ON 1100ms / OFF 1100ms (BATT_CRITICAL, MAPPING_NO_ISD, OTA_QCC)
  - ON 80ms / OFF 220ms × 5회 (POWER_ON 버스트)
  - ON 100ms / OFF 200ms × 4회 (POWER_OFF 버스트)
  - 지속 ON: BATT_READY / IN_USE / BATT_MID / MAPPING_ISD → DC 레벨로 확인
- 시나리오:
  - (a) 초기화 OK → POWER_ON 버스트 → BATT_READY (녹색 지속 ON).
  - (b) 초기화 실패 → ERROR 즉시.
  - (c) 운용 중 ERROR 주입 → 기존 표시 선점.
  - (d) POWER_OFF 요청 → ERROR보다 우선.
  - (e) mapping 연결 + ISD 미연결 → 파랑 ON 1100ms / OFF 1100ms 점멸.
  - (f) mapping 연결 + ISD 연결 → 파랑 지속 ON.
  - (g) 배터리 9% + ISD 연결 → BATT_CRITICAL (노랑 ON 1100ms / OFF 1100ms 점멸). 사용자 off → OFF.
  - (h) 배터리 9% + ERROR → ERROR (상위 prio).
  - (i) OTA_QCC 수신 → 녹색 ON 1100ms / OFF 1100ms 점멸.
  - (j) OTA_EZAIRO 수신 → 녹색 ON 180ms / OFF 180ms 점멸 (QCC와 구분).
  - (k) PAIR 100ms 짧은 이벤트 → 500ms latch로 표시 확인.
  - (l) 배터리 30% (BATT_MID) → 노랑 지속 ON, 점멸 없음 확인.
...
- Mapping 연결은 파랑 ON 1100ms / OFF 1100ms 점멸(ISD 미연결) 또는 파랑 지속 ON(ISD 연결)으로 BATT_CRITICAL을 선점.
- Rev.5 패턴 확정: BATT_CRITICAL = 노랑 ON 1100ms / OFF 1100ms 점멸, BATT_MID = 노랑 지속 ON, BATT_READY = 녹색 지속 ON, OTA_QCC = 녹색 ON 1100ms / OFF 1100ms 점멸, OTA_EZAIRO = 녹색 ON 180ms / OFF 180ms 점멸.
