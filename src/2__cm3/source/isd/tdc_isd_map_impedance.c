...
static void tdc_isd_map_impedance_flow00_calc_frame_and_dac(void)
{
    ...
    // 최대 전하량 계산
    deliveryCharge_pico = (s_mappingPacket->impedanceCheck.pulseWidth_end_usec) * (s_mappingPacket->impedanceCheck.stimulationLevel_uA);  // usec단위.. (time*10^-6}*{level*10^-6)
    ...
    {
        if (s_mappingPacket->impedanceCheck.stimulationLevel_uA > stimulDAC_D_only_Saturation_uA)  // 오프셋을 적용하지 않은 상태에서 출력할 수 없으면 오프셋 적용
        {
