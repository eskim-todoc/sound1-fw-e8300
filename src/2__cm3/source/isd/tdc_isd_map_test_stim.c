...
bool testStimulation(bool startFlag)
{
    ...
    switch (flowControlCounter)
    {
        case 0:
        {
            ...
            numFramePerChannel = tdc_stim_calc_frame_per_channel(mappingPacket->testStimulation.pulseWidth);

            // 사용가능한 채널 수는 24채널 보다 적고 펄스폭은 길게 설정할 경우... 가능하다면 1msec에 1회의 출력이 나올 수 있도록..

            TransferabelChannelNum = tdc_stim_calc_transferable_channel_num(numFramePerChannel, mappingPacket->testStimulation.usableElectrodeNum);
        ...
        case 20: // I2C 읽기 - 바이폴라 기준 전극 값 확인 - FIFO 값 확인..
        {
            ...
            if (tdc_isd_fpga_read_fifo_counter(&r_FPGA_registerValue)) // 0
            {
                ...
                if (r_FPGA_registerValue != 0)
                {
                    ...
                    if (tdc_isd_fpga_read_backtel_fifo(backtelBuff, df_MaxNumOfElectrode))
                    {
        ...
        case 25: // 자극 파라미터 설정  확인
        {
            ...
            if (tdc_isd_fpga_read_fifo_counter(&r_FPGA_registerValue))
            {
                ...
                else
                {
                    ...
                    tdc_isd_fpga_read_backtel_fifo(backtelBuff, 2);

                    tdc_isd_fpga_read_system_error_flag(&r_FPGA_registerValue);
        ...
        case 26: // 자극용 파라미터 설정 시작(펄스 폭 설정)
        {
            ...
            // 펄스 폭 조정
            tdc_isd_fpga_change_pulse_width(pcm_index++, mappingPacket->testStimulation.pulseWidth);
    ...
    if (flowControlCounter > cofigureDoneCounter) // 자극 설정이 완료되었으며 자극출력을 시작한다.
    {
        ...
        else // 자극 시간 동안 자극 출력
        {
            ...
            for (i = 0; i < TransferabelChannelNum; i++) // 1msec 동안 출력할 수 있는 채널 수
            {
                ...
                tdc_shm_fill_specific_command_buffer(pcm_index++, stimulPCM_buff[trasnferChannel_Index++]);
