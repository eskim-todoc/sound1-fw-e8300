...
bool tdc_isd_set_stim_para_monopolar(bool isdControlStateChagedFlag)
{
    ...
    switch (flowControlCounter)
    {
        ...
        case 10:
        {
            ...
            tdc_isd_fpga_change_pulse_width(pcm_index++, p_mapdata->stimulationPulsePhaseWidth);
        ...
        case 14:
        {
            if (tdc_isd_fpga_read_pulse_width(&r_FPGA_registerValue))
            {
                ...
                {
                    tdc_isd_fpga_update_fpga_pulse_phase_width_written_value(p_mapdata->stimulationPulsePhaseWidth);  //
                    // 자극 파라미터 설정 완료
    ...
    switch (flowControlCounter)
    {
        ...
        case 29:
        {
            ...
            if (tdc_isd_fpga_read_fifo_counter(&r_FPGA_registerValue))
            {
                if (r_FPGA_registerValue != 0)
                {
                    ...
                    if (tdc_isd_fpga_read_backtel_fifo(backtelBuff, r_FPGA_registerValue))
                    {
        ...
        case 39:
        {
            if (tdc_isd_fpga_read_pulse_width(&r_FPGA_registerValue))
            {
                if (r_FPGA_registerValue == p_mapdata->stimulationPulsePhaseWidth)
                {
                    tdc_isd_fpga_update_fpga_pulse_phase_width_written_value(p_mapdata->stimulationPulsePhaseWidth);  // 실제로는 Fpag 값을 읽어보고 업데이트 해야 된다.
                    // 자극 파라미터 설정 완료
