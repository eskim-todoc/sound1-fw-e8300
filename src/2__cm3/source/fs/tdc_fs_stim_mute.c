...
int tdc_fs_stim_mute_init(void)
{
    ...
    fp    = &g_tdc_fs_ohdl;
    fname = TDC_FS_STIM_MUTE_FILE_NAME;
    ...
    br  = 0;
    ret = f_read(fp, (void *) &stim_mute, sizeof(TDC_FS_STIM_MUTE_T), &br);
    ...
    if (br != sizeof(TDC_FS_STIM_MUTE_T))
    {
        is_validated_file = false;
    }
    else
    {
        ...
        if ((stim_mute.file_ident_begin != TDC_FS_STIM_MUTE_FILE_IDNET_BEGIN)  // IDENT가 하나라도 일치 하지 않으면 에러
            || (stim_mute.file_ident_end != TDC_FS_STIM_MUTE_FILE_IDENT_END))
        {
            is_validated_file = false;
        }

        // is_enabled_mute_stimulation_under_t_level, mute_stimulation_t_level_offset 값의 유효성 확인
        if ((stim_mute.is_enabled_mute_stimulation_under_t_level != TDC_FS_STIM_MUTE_UNDER_T_LEVEL_ENABLE)  //
            && (stim_mute.is_enabled_mute_stimulation_under_t_level != TDC_FS_STIM_MUTE_UNDER_T_LEVEL_DISABLE))
        {
            is_validated_file = false;
        }

        // T LEVEL OFFSET MIN 이상이 아니면 유효성 에러
        if (!(TDC_FS_STIM_MUTE_T_LEVEL_OFFSET_MIN <= stim_mute.mute_stimulation_t_level_offset))
        {
            is_validated_file = false;
        }
        else
        {
            ...
            if (!(stim_mute.mute_stimulation_t_level_offset <= TDC_FS_STIM_MUTE_T_LEVEL_OFFSET_MAX))
            {
                is_validated_file = false;
            }
        }
    ...
    if (!is_validated_file)
    {
        ...
        // 구조체 내용 초기화
        // TDC_FS_STIM_MUTE_T_LEVEL_OFFSET_DEFAULT 값은 CFX 프로젝트에서 참조
        stim_mute.file_ident_begin                          = TDC_FS_STIM_MUTE_FILE_IDNET_BEGIN;
        stim_mute.is_enabled_mute_stimulation_under_t_level = TDC_FS_STIM_MUTE_UNDER_T_LEVEL_DISABLE;
        stim_mute.mute_stimulation_t_level_offset           = TDC_FS_STIM_MUTE_T_LEVEL_OFFSET_DEFAULT;
        stim_mute.file_ident_end                            = TDC_FS_STIM_MUTE_FILE_IDENT_END;

        f_lseek(fp, 0);
        ret = f_write(fp, (void *) &stim_mute, sizeof(TDC_FS_STIM_MUTE_T), &bw);

        if ((ret != FR_OK) || (bw != sizeof(TDC_FS_STIM_MUTE_T)))
        {
...
int tdc_fs_stim_mute_update(uint32_t enable, uint32_t level)
{
    ...
    // 매개변수 유효성 확인
    if ((TDC_FS_STIM_MUTE_UNDER_T_LEVEL_ENABLE == enable) || (TDC_FS_STIM_MUTE_UNDER_T_LEVEL_DISABLE == enable))
    {
    ...
    else
    {
        ...
        return TDC_FS_STIM_MUTE_RET_FALSE;
    }

    if ((TDC_FS_STIM_MUTE_T_LEVEL_OFFSET_MIN <= level) && (level <= TDC_FS_STIM_MUTE_T_LEVEL_OFFSET_MAX))
    {
