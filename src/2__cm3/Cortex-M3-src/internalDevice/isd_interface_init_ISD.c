...
int read_Connected_ISD_id(void)
{
    return isd_id;
}

void isd_path_Open(bool isdControlStateChagedFlag)
{
    ...
    switch (flowControlCounter)
    {
        case 0:  // FPGA 에러 발생 상태 확인 후 PCM 펄스폭 최소로 설정 (이 case에서 PCM 모드 변경 예정)
        {
            ...
            else
            {
                ...
                // 앞선 isd_interface 함수 과정에서
                // init_ISD() 함수를 통해 내부기 칩의 전원 레벨을 읽고 난 후 이 함수를 수행하기 때문에
        ...
        {
            temporal_registerValue = change_8BitBacktel_mode(pcm_index++);
...
#ifndef DisalbedBackTel
            // FIFO 카운터를 읽어 본다.
            if (read_FPGA_FIFO_counter(&FIFO_CounterValue))
            {
                if (FIFO_CounterValue == NumByte_ISD_chip_ID)
                {
                    ...
                    if (read_FPGA_backtel_FIFO(r_FPGA_FIFO_buff, NumByte_ISD_chip_ID))
                    {
                        ...
                        if (isd_id != 0)
                        {
                            ...
                            if (p_mappingPacket->command != en__mapping_write_original_ISD_N_USER)
                            {
                                ...
                                if (isNormalUser)
                                {
                                    ...
                                    {
                                        isd_id_onFlash = read_ISD_manufacture_ID(i);
        ...
        case 27:  // PCM 출력 - 해당 ID의 passkey를 내부기에 전달하고, Path가 열렸는지 확인한다.
        {
            ...
            if (isNormalUser)
            {
                isd_id_onFlash = read_ISD_manufacture_ID(isd_id_match_num - 1);
            ...
            // 내부기 열렸는지 읽기
            fill_pcmBuff_check_ISD_PathOpen_normalValue(&pcm_index);
            ...
            if (read_FPGA_FIFO_counter(&FIFO_CounterValue))
            {
                if (FIFO_CounterValue == 1)
                {
                    if (is_arbitraryValue_matched_normalValue())
                    {
                        ...
                        fill_pcmBuff_check_ISD_PathOpen_duplicateZeroData(&pcm_index);
...
#else
            clearAllErrorFlag();
            change_isd_state(en__isdStatus_ISD_pathOpen_Ok);
            changeConnected_isd_num_CFX(isd_id_match_num);  // CFX와의 공유 메모리에 연결된 내부기 번호(1~4)를 알려주어서
        ...
        case 35:  // I2C 읽기 - 0으로 채워진 값으로 백텔 검증이 통과하면, 해당 ISD 번호가 진짜 연결된 것으로 간주함
        {
            if (read_FPGA_FIFO_counter(&FIFO_CounterValue))
            {
                if (FIFO_CounterValue == 1)
                {
                    if (is_arbitraryValue_matched_duplicateZeroData())
                    {
                        // 정상
                        clearAllErrorFlag();
                        change_isd_state(en__isdStatus_ISD_pathOpen_Ok);
                        ...
                        {
                            changeConnected_isd_num_CFX(isd_id_match_num);
                        }
                        else
                        {
...
void enableStimul_10v(bool isdControlStateChagedFlag)
{
    static int flowControlCounter = 0;
    ...
    switch (flowControlCounter)
    {
        ...
        case 0:
        {
            ...
            else
            {
                ...
                // 앞선 isd_interface 함수 과정에서
                // isd_path_Open() 함수를 통해 내부기 칩의 암호키 복호화 및 Forward Check 레지스터 검증까지 완료되었으므로
                // 마지막 PCM 동작 모드는 NopStandby 일 것이다.
                // 그래서 여기서는 바로 SpecificCommand를 적용시켜도 정상 동작한다.
        ...
        case 4:  // 펄스폭 최소 설정 및 백텔 레지스터 8비트 모드 설정 확인
        {
            ...
            if (read_FPGA_backtelConfig(&r_FPGA_registerValue))
            {
                if (writenBacktelRegisterValue == r_FPGA_registerValue)
                {
                    ...
                    if (write_FPGA_clear_FIFO())  // 피포를 지우고
                    {
                        // FPGA 상태를 읽어 본다.
        ...
        case 28:
        {
            ...
            if (read_FPGA_FIFO_counter(&r_FPGA_registerValue))
            {
                ...
                {
                    if (read_FPGA_backtel_FIFO(&r_isd_registerValue, 1))
                    {
                        vtg_Lock_state = data_ExtractionAndRigthShift(r_isd_registerValue, bitPosition_VTG_LOCK, 1);
#if 0
                        vtg_Lock_state=0;
#endif
                        if (vtg_Lock_state == 1)
                        ...
                        else
                        {
                            ...
                            {
                                errorCodeUpdate(en__EN__ISD_ERROR, en__VTG_Lock_Error, __LINE__);
                                change_isd_state(en__isdStatus_FPGA_Ok);  // 내부기 전송 파워 설정 부터 다시.
                            }
#endif
                ...
                {
                    read_FPGA_systemResgister_1st(&r_FPGA_registerValue);
