...
#define TDC_TOUCH_POLL_INTERVAL_MS   50    /* 폴링 주기 (ms) */
...
#define TDC_TOUCH_BOOT_RELEASE_DEBOUNCE_MS 400  /* 부팅 터치 해제 디바운스 (ms) — 연속 not-pressed 유지 */
...
#define TDC_TOUCH_ULP_NOTOUCH_DEBOUNCE_MS 400    /* 첫 노터치 확정(게이트 해제) 연속 시간 */
 ...
 * 공식: T[ms] = 2^PRESCALE x (TIMEOUT+1) / 40  (SLOWCLK 40kHz). 2^7 x 63 / 40 = 201.6ms. */
