 ...
 * tdc_fs_gain.c
 *
 * Gain Conversion Table 인덱스의 ISD별 영속화. 상세는 tdc_fs_gain.h 참조.
 */
...
int tdc_fs_gain_load(int isd_num, tdc_fs_gain_setting_t *p_out)
{
    if (p_out == NULL)
    {
        return TDC_FS_GAIN_RET_FAIL;
    }

    // 어떤 경로로 실패하더라도 호출부가 쓸 수 있도록 먼저 기본값을 채워 둔다.
    p_out->gain_table_index_a = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_A;
    p_out->gain_table_index_b = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_B;

    if ((isd_num < 1) || (MaxNumUser < isd_num))
    {
        TDC_PRINTF_W("[GAIN] LOAD FAIL (INVALID ISD NUM: %d) \r\n", isd_num);
        return TDC_FS_GAIN_RET_FAIL;
    }

    if (tdc_gain_load_and_repair() != TDC_FS_GAIN_RET_OK)
    {
        // 복구까지 실패한 경우로, 기본값을 그대로 쓴다.
        TDC_PRINTF_W("[GAIN] LOAD FAIL (ISD %d), USE DEFAULT \r\n", isd_num);
        return TDC_FS_GAIN_RET_FAIL;
    }

    *p_out = m_tdc_gain_file.setting[isd_num - 1];

    TDC_PRINTF_I("[GAIN] LOADED : ISD %d, A %d, B %d \r\n", isd_num, p_out->gain_table_index_a, p_out->gain_table_index_b);

    return TDC_FS_GAIN_RET_OK;
}
...
int tdc_fs_gain_save(int isd_num, const tdc_fs_gain_setting_t *p_in)
{
    ...
    if (tdc_gain_write_file() != TDC_FS_GAIN_RET_OK)
    {
        TDC_PRINTF_W("[GAIN] SAVE FAIL (WRITE ERROR, ISD %d) \r\n", isd_num);
        return TDC_FS_GAIN_RET_FAIL;
    }

    TDC_PRINTF_I("[GAIN] SAVED : ISD %d, A %d, B %d \r\n", isd_num, p_in->gain_table_index_a, p_in->gain_table_index_b);
...
int tdc_fs_gain_reset(int isd_num)
{
    ...
    gain_setting.gain_table_index_a = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_A;
    gain_setting.gain_table_index_b = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_B;

    TDC_PRINTF_I("[GAIN] RESET TO DEFAULT : ISD %d \r\n", isd_num);

    return tdc_fs_gain_save(isd_num, &gain_setting);
}

static bool tdc_gain_is_valid_index(int index)
{
    return ((TDC_FS_GAIN_TABLE_INDEX_MIN <= index) && (index <= TDC_FS_GAIN_TABLE_INDEX_MAX));
}

static void tdc_gain_set_default_all(void)
{
    m_tdc_gain_file.ident_begin = TDC_FS_GAIN_FILE_IDENT_BEGIN;
    m_tdc_gain_file.ident_end   = TDC_FS_GAIN_FILE_IDENT_END;

    for (int i = 0; i < MaxNumUser; i++)
    {
        m_tdc_gain_file.setting[i].gain_table_index_a = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_A;
        m_tdc_gain_file.setting[i].gain_table_index_b = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_B;
    }
}
...
static int tdc_gain_load_and_repair(void)
{
    ...
    if (tdc_gain_read_file() != TDC_FS_GAIN_RET_OK)
    {
        // 파일이 없거나 CRC 가 깨진 경우로, 전 슬롯을 기본값으로 만든다.
        TDC_PRINTF_W("[GAIN] FILE READ FAIL, RESET ALL SLOTS \r\n");
        tdc_gain_set_default_all();
        need_rewrite = true;
    }
    else if ((m_tdc_gain_file.ident_begin != TDC_FS_GAIN_FILE_IDENT_BEGIN)  //
             || (m_tdc_gain_file.ident_end != TDC_FS_GAIN_FILE_IDENT_END))
    {
        // 크기는 맞지만 우리 파일이 아닌 경우다.
        TDC_PRINTF_W("[GAIN] FILE IDENT MISMATCH, RESET ALL SLOTS \r\n");
        tdc_gain_set_default_all();
        need_rewrite = true;
    }
    else
    {
        ...
        for (int i = 0; i < MaxNumUser; i++)
        {
            ...
            {
                TDC_PRINTF_W("[GAIN] INVALID INDEX A (SLOT %d: %d), RESET \r\n", i + 1, m_tdc_gain_file.setting[i].gain_table_index_a);
                m_tdc_gain_file.setting[i].gain_table_index_a = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_A;
                need_rewrite                                  = true;
            }
            ...
            {
                TDC_PRINTF_W("[GAIN] INVALID INDEX B (SLOT %d: %d), RESET \r\n", i + 1, m_tdc_gain_file.setting[i].gain_table_index_b);
                m_tdc_gain_file.setting[i].gain_table_index_b = TDC_FS_GAIN_TABLE_INDEX_DEFAULT_B;
                need_rewrite                                  = true;
            }
        }
    ...
    if (need_rewrite)
    {
        if (tdc_gain_write_file() != TDC_FS_GAIN_RET_OK)
        {
            TDC_PRINTF_W("[GAIN] REPAIR WRITE FAIL \r\n");
            return TDC_FS_GAIN_RET_FAIL;
        }
    }
