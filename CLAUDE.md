...
**TL;DR**: E8300 임베디드 펌웨어 프로젝트. 루트 헌법/지침(글로벌 `~/.claude/CLAUDE.md`가 지정하는 `E:\workspace\rules\CLAUDE.md` 및 `E:\workspace\rules\지침\`)을 **절대 우선**으로 따른다. 본 파일은 Sound1 고유 정보(repo·기여 흐름·폴더 구조·작업 라우팅)만 담는다.

> [!IMPORTANT]
> **Sound1은 루트 헌법/지침을 절대 우선으로 따른다.** 루트 진입점은 글로벌 `~/.claude/CLAUDE.md`가 지정하는 `E:\workspace\rules\CLAUDE.md`이며, 그 지침 체계(`E:\workspace\rules\지침\`)를 상속한다. 본 파일은 Sound1 고유 정보(repo·메타·기여 흐름·폴더 구조·작업 라우팅)만 담고, 일반 규칙은 모두 루트에서 상속한다.
>
> **세션 시작 시**: ① 루트 `E:\workspace\rules\CLAUDE.md` 명시 Read → "루트 지침 확인 완료: <한 줄 요약>" 보고, ② 루트 `E:\workspace\rules\지침\작업 규칙.md` 확인.
>
> **지침 단일화 (2026-07-14~)**: 프로젝트 자체 `docs/지침/` 폴더는 **비활성(deprecated)** — 모든 지침은 루트만 따른다. 도메인 특화 지침이 필요하면 루트로 승격해 관리한다([`docs/지침/README.md`](docs/지침/README.md) 참조).
>
> **충돌 정책**: 본 CLAUDE.md ↔ 루트 CLAUDE.md 충돌 시 → **루트 우선**. 본 파일이 잘못된 것으로 간주, 사용자에게 즉시 보고.
...
└── docs/           SW 문서 — 루트 컨벤션 적용 (참고/·사용방법/·tasks/<모듈>/<작업>/; 지침/은 비활성)
```

## 작업 라우팅

루트 지침(`작업 규칙.md §3`)의 **2계층 tasks 모델**을 따른다.

- **모듈 현황**: [`docs/tasks/모듈 현황.md`](docs/tasks/모듈 현황.md) — 모듈별 최신 작업 인덱스 (신규 작업 착수 전 먼저 조회)
- **모듈별 작업 목록**: `docs/tasks/<모듈>/작업 목록.md` — 신규 작업은 해당 모듈 목록에 등재하고 `<모듈>/YYYYMMDD_<슬러그>/` 폴더 생성
- 기존 통합 [`docs/tasks/작업 목록.md`](docs/tasks/작업 목록.md)(단일 표)는 **deprecated·동결** — 과거 이력 참조용
