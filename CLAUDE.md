...
> 작업 전 반드시 [`지침/작업 규칙.md`](../../docs/지침/작업%20규칙.md)를 확인할 것.
...
- Git Workflow: 루트 [`지침/Git/브랜치, 병합 규칙.md`](../../docs/지침/Git/브랜치,%20병합%20규칙.md) 참조
...
└── docs/           SW 문서 (지침/·참고/·사용방법/·tasks/<모듈>/<작업>/)
```
...
| 항목 | 참조 문서 |
|---|---|
| 네이밍 컨벤션 | [`지침/코딩/네이밍 컨벤션.md`](../../docs/지침/코딩/네이밍%20컨벤션.md) |
| 문서 작성 규칙 | [`지침/일반/문서 작성 규칙.md`](../../docs/지침/일반/문서%20작성%20규칙.md) |
| 프로그래밍 작업 규칙 | [`지침/코딩/작업 규칙.md`](../../docs/지침/코딩/작업%20규칙.md) |
| Git 규칙 | [`지침/Git/브랜치, 병합 규칙.md`](../../docs/지침/Git/브랜치,%20병합%20규칙.md) |

## 문서 작성 (프로젝트 고유)

### 내부 작업 문서 (이 repo 내)

루트 [`지침/일반/문서 작성 규칙.md`](../../docs/지침/일반/문서%20작성%20규칙.md) 컨벤션 적용. 현재 `docs/` 레이아웃:

```
docs/
├── 지침/     프로젝트 특화 지침 (레지스터 검증 체크리스트 등 — 전역 지침은 루트 참조)
├── 참고/     외부 스펙·도메인 지식 (모듈별 분할)
│   ├── 칩/   Ezairo FW/HW Reference, IQS323 레지스터 등 외부 칩 레퍼런스
│   ├── LED/  LED 시스템 도메인 문서 (Dimming · 색감 · 아키텍처)
│   └── 터치/ 터치 센서 도메인 문서
├── 사용방법/ UI 명령어 등 사용법
└── tasks/
    ├── list.md                       작업 레지스트리
    └── <모듈>/<작업슬러그>/          요구사항·분석·구현계획·진행상황(선택)·이력(선택)
```

활성·완료 작업 목록은 [`tasks/list.md`](docs/tasks/list.md) 참조.

### 대외 제출용 공식 문서 (Dropbox 공유)
